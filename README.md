...
### 깜빡임을 넣으려면 (아직 안 만들었습니다)

원리는 확인됐습니다. 클립에 이런 커브를 추가하면 됩니다:

```
path      : body
type      : SkinnedMeshRenderer (classID 137)
attribute : blendShape.zepeto.eyeBlinkLeft   (그리고 ...Right)
값        : 0 = 뜬 눈, 100 = 감은 눈
```

Blender는 이걸 만들 수 없고(모델에 모양이 없으니), **Unity 쪽에서 추출된 `.anim`에 직접 넣어야**
합니다. 지금 이 저장소에는 그 기능이 **없습니다.** 있다고 적어 두는 것보다 없다고 적는 편이
낫다고 판단했습니다.
