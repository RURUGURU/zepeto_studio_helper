...
> **남은 2장은 사람이 결정해야 한다.**
> `play-preview.png`는 성질상 자동으로 못 고친다 — 그 이미지의 존재 이유가 "실제 아바타가 로드된 화면"인데
> placeholder 아이디로는 아바타가 아예 나오지 않는다. 버리거나 별도 계정으로 찍어야 한다.
> `workflow-overview.png`는 `1 -> 2 -> 3 -> 4 작업 흐름`을 전제로 **설계된 합성 도해**다. 4단계 전제가
> 틀렸고, 제거된 단계 잠금을 설명하고, `PLAY` 칸에 아바타가 들어 있다. 도해를 다시 만들어야 하므로
> README 상단 임베드만 내리고 파일은 남겨뒀다.
