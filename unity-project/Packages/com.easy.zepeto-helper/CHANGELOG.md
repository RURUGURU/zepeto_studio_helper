...
## 0.11.1 (미출시) — 깜빡임 버튼이 남의 눈 애니메이션을 지울 수 있었다

0.11.0의 `빼기`는 "커브가 있으면 우리 것"으로 판단했다. 그런데 **ZEPETO 공식 동작은 이미 같은
커브를 갖고 있다.** 2번 카드의 기본값인 Videobooth_282를 복사해 오면 그 사본에
`blendShape.zepeto.eyeBlinkLeft`가 키 **95개**(값 10~100), Right가 97개 들어 있다. 춤에 맞춰 손으로
만든 눈꺼풀 연기다.

그래서 6번 카드를 **처음 여는 순간** 상태가 "깜빡임 있음"이 되고 `빼기`가 눌리는 상태였다. 한 번
누르면 그 95개가 사라졌다. Undo도 없었고(`SetEditorCurve`는 Undo에 기록되지 않는다), 2번을 다시
눌러도 `FindCopiedAnimationForPackage`가 망가진 사본을 그대로 다시 집었다. `다시 넣기`도 같은
일을 했다 - 3.4초 간격의 기계적인 커브로 갈아치웠다. 게다가 형제 보정 커브 6종
(`eyeBlinkLeftEyeSquintLeft` 등, 각 95~248키)은 남아서 눈꺼풀 없이 보정만 도는 상태가 됐다.

이 프로젝트가 스스로 세운 규칙과도 어긋났다: `ClipEdit.cs`는 "clip 조정은 copy-on-write다 -
package/cache clip도, 기존 작업 clip도 제자리에서 편집하지 않는다"고 적어 두었는데 이 기능만
제자리에서 고쳤다.

**모양으로 주인을 판별한다.** 우리가 쓰는 커브는 값이 0 아니면 100이고 키가 정확히
`2 + 3 × 깜빡임 수`개다. 공식 커브는 그 사이 값을 촘촘히 갖는다. 하나라도 어긋나면 `Authored`로
보고 **넣기와 빼기 둘 다 잠근다.** 왜 잠갔는지도 카드 안에 적는다 - 눌리는데 아무 일도 안 나면
고장으로 읽히기 때문이다.

되돌리기도 붙였다(`Undo.RegisterCompleteObjectUndo`). 판별이 유일한 안전장치인 상태로 두지 않는다.

`ZepetoBlinkRun`에 `blink:refuses-to-overwrite-authored`를 넣어 실제 SDK 클립에 대고 확인한다.
결과: **키 95 → 95, 반환 0** — 한 개도 건드리지 않았다.


## 0.11.0 (미출시) — 눈 깜빡임

6번 카드에 `얼굴 · 눈 깜빡임` 칸이 생겼다. 버튼 한 번이면 연결된 클립에 깜빡임을 넣고, `빼기`로
되돌린다. `package.json`은 `0.11.0`, 애드온은 `1.5.1` 그대로다 — Blender 쪽은 한 글자도 안 바뀌었다.

### 왜 Blender가 아니라 Unity인가

깜빡임은 뼈가 아니라 블렌드셰이프다. 그리고 3번이 내보내는 몸에는 그 모양이 **하나도 없다**
(`ZepetoBaseModel.prefab`의 `m_Shapes`가 메시 4개 전부 비어 있다). Blender에는 섞을 것이 없으니
키를 찍을 수도 FBX에 실을 수도 없다. 얼굴 모양 250개는 Play 중 서버에서 아바타가 내려올 때 붙는다.

그래서 커브는 추출된 `.anim`에 Unity 쪽에서 직접 쓴다. 바인딩은 지어내지 않고 ZEPETO가 자기 공식
모션(`zepeto.studio@3.2.16`의 `ANI_039_v03.anim`)에서 쓰는 것을 그대로 읽어 왔다:
...
이 문자열이 한 글자만 달라도 커브는 저장되고, 재생되고, **아무 일도 하지 않는다.** 에러도 경고도
없다. 그래서 검증을 눈으로 하지 않고 수치로 했다.

### 진짜 아바타에서 눈이 감기는 것을 쟀다

`ZepetoBlinkRun`이 Play 중 `SkinnedMeshRenderer.GetBlendShapeWeight`를 매 프레임 읽는다.
결과는 **0.0 → 99.3 → 0.0**. 커브가 존재한다는 것이 아니라 눈꺼풀이 실제로 움직인다는 증명이다.

그 러너의 첫 실행은 `FAIL blink:eye-closes :: 최대 0.0`을 냈다. 기능이 아니라 하네스의 실패였다 —
깜빡임은 `BlinkTarget.anim`에 넣고 재생 슬롯에는 `DanceDemo10s.anim`이 물려 있었다. 재생을 정하는
것은 LOADER의 clip 필드가 아니라 AnimatorOverrideController의 슬롯이라는, 이 프로젝트가 이미
여러 번 밟은 함정이다. 러너가 헬퍼의 `AssignAnimationClip`을 부르도록 고쳤고, 그 단계 자체도
`blink:clip-assigned-to-playback-slot`으로 검사한다.

### 곡선의 모양

- 간격 3.4초. 사람은 3~5초에 한 번 깜빡인다.
- 한 번에 0.13초. 감는 데 40%, 뜨는 데 60% — 반반으로 주면 기계처럼 보인다.
- 0초와 클립 끝에 값 0을 못박는다. 이 키가 없으면 Unity가 바깥으로 값을 연장해서, 반복 재생할 때
  눈이 감긴 채로 다음 바퀴로 넘어간다.
- 클립이 너무 짧으면 넣지 않고 이유를 말한다. 억지로 하나 끼우면 1초짜리 클립 내내 눈을 감고 있다.

### 알아야 할 두 가지 (버튼 옆에도 적어 두었다)

1. **편집 화면에서는 안 보인다.** 회색 기본 몸에 표정 모양이 없다. Play를 켜야 보인다.
2. **Blender에서 다시 내보내면 사라진다.** 이 커브는 Unity 쪽 `.anim`에만 있다. 마지막에 넣어야 한다.


## 춤을 한 곡 실제로 만들어 봤다 (미출시)

**버전은 아무것도 올리지 않았다.** 애드온 코드도 헬퍼 패키지 코드도 한 글자 안 바뀌었다 — 새로 생긴
것은 도구를 *쓰는* 스크립트(`make_dance.py`)와 문서·이미지뿐이다. 안 바뀐 것을 바뀌었다고 적을 수는
없다. 애드온은 `1.5.1`, 패키지는 `0.10.1` 그대로다.

"만들 수 있다"와 "만들어 봤다"는 다른 문장이다. 검사 묶음들은 버튼 하나하나가 약속대로 도는지를
증명하지만, 그것들을 엮어서 사람이 춤이라고 부를 만한 것이 나오는지는 증명하지 않았다.

`BlenderMotion/make_dance.py`가 그 하나를 증명한다. 240프레임 20비트(120 BPM 기준 한 비트 = 정확히
12프레임) 안무를 끝까지 찍고, 루프를 닫고, 애드온의 게이트를 그대로 통과해서 내보낸다. 우회 경로는
쓰지 않는다 - `key_pose` / `make_loop` / `locate_paths` / `export` 오퍼레이터만 부른다.

| 무엇 | 실측값 |
| --- | --- |
| 길이 | 240프레임 @ 24fps = 10.0초 → Unity 임포트 후 **9.96초** Humanoid 클립 |
| 루프 이음새 | 첫/끝 프레임 최대 각도차 **0.00e+00 rad** |
| 손 이동 (Blender 한 바퀴) | 왼손 7.82m / 오른손 7.77m, 머리 2.25m, 양발 각 1.20m |
| Unity Play 중 실제 아바타 | 오른손이 골반 기준 **0.654m** 이동 |

### 뼈 회전축을 처음으로 문서화했다

안무를 짜려면 "이 뼈의 Y를 돌리면 무엇이 어디로 가는가"를 알아야 하는데, 어디에도 적혀 있지 않았다.
렌더와 수치 측정으로 전부 확인해서 `make_dance.py` 상단에 표로 남겼다. 알아낸 것 중 안 적혀 있으면
반드시 헤매게 되는 것 셋:

- rest pose가 **T자세**다. 팔을 내리려면 Z를 -70도 근처까지 줘야 하고, 수평 근처에 두면 '포즈를 안
  잡은 것'과 실루엣이 구별되지 않는다.
- 몸통(`spine`/`chest`)의 **X는 몸통 회전**이다. 머리가 그 회전축 위에 있어서 정면에서는 거의 보이지
  않는다 - 회전을 눈에 보이게 하려면 팔을 몸 앞으로 가로질러야 한다.
- **웅크리기는 불가능하다.** 골반이 내려가야 하는데 골반은 뼈가 아니라 아마추어 오브젝트이고, 그것을
  움직이는 것은 이 파이프라인이 금지하는 동작이다(아바타가 화면 밖으로 걸어나간다). 무릎만 굽히면
  정면에서 아무 일도 일어나지 않는다.

첫 판에서 20비트 중 6비트가 실제로 이 함정에 빠져 있었다 - 렌더해서 눈으로 보기 전까지는 13개 검사가
전부 초록이었다. 키프레임 개수와 이동 거리는 '움직인다'를 증명하지만 '춤처럼 보인다'를 증명하지 않는다.

### README에 움직이는 그림을 넣었다

`docs/images/dance-demo.gif` — 120프레임(2프레임 간격), 12fps 재생이라 원래 속도 그대로다. 0.31MB.
모션 도구의 README에 정지 화면만 있는 것이 이상했다.

특정 곡의 실제 안무는 저작물이라 옮기지 않았다. 위 동작은 창작이고, 120 BPM 곡이면 어디에 붙여도
박자가 맞는다.
...
이 검사는 배포되는 그 `.blend`을 그대로 열고 `README.md`의 순서대로만 누른다. 규칙은
...
`README.md`는 제목이 "초보자용 전체 가이드"인데, 끝이 "문제없으면 6번 → 7번 제페토로
