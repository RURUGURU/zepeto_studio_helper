...
| --- | --- | --- |
| `eye_L` · `eye_R` | 눈 | `R` → `X`로 위아래(감았다 뜨기), `Z`로 좌우(눈동자) |
| **`mouth`** | 턱 | `R` → `X`로 입 벌리기 |

> ⚠️ **턱은 `jaw`가 아니라 `mouth`입니다.** `jaw`라는 뼈도 있지만 Unity 매핑이 없어서 숨겨져 있고,
> 돌려도 조용히 사라집니다. `nose` · `lip_L` · `lip_R`도 마찬가지입니다.

**웃는 표정 같은 건 못 만듭니다.** 표정은 보통 블렌드셰이프(모프)로 만드는데 이 몸에는 그게
**0개**이고, Humanoid 클립은 애초에 블렌드셰이프를 담지 못합니다. 여기서 되는 얼굴 연기는
**눈 감았다 뜨기 · 눈동자 좌우 · 입 벌리기** 세 가지입니다.
