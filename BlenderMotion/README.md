...
패널 맨 위의 **`처음부터 자세히 보기`** 는 기본으로 펼쳐져 있습니다. 화면 회전·이동·확대,
파란 뼈 선택과 `R` 회전, 축 고정, 취소와 되돌리기, 자주 쓰는 뼈 이름, 키프레임 저장,
루프와 Unity 전송까지 Blender 안에서 순서대로 읽을 수 있습니다. 익숙해지면 제목을 눌러 접어도
아래 5단계 작업 버튼은 그대로 남습니다.
...
> ⚠️ **턱은 `jaw`가 아니라 `mouth`입니다.** `jaw`라는 뼈도 있지만 Unity 매핑이 없어 숨겨져 있고,
> 돌려도 조용히 사라집니다. `nose` · `lip_L` · `lip_R`도 마찬가지입니다.

얼굴 뼈는 아주 작아서 화면을 확대(마우스 휠)해야 잡기 편합니다.

#### 눈은 왜 안 깜빡이나

**눈꺼풀 뼈가 없기 때문입니다.** `eye_L`을 돌리면 움직이는 것은 눈알 385개 정점뿐이라
눈동자가 어디를 보는지만 바뀝니다.

그런데 **제페토 아바타 자체는 깜빡입니다.** Play 중 실제 아바타를 조사해 보면 `body` 메시에
블렌드셰이프(미리 만들어 둔 표정 모양)가 **250개** 있고, 그중 `zepeto.eyeBlinkLeft` ·
`zepeto.mouthSmileLeft` · `zepeto.browInnerUp` 같은 깜빡임·표정용이 79개입니다.
제페토 공식 모션도 그 커브를 57종 담고 있습니다.

**빠진 곳은 Blender 쪽 몸입니다.** 3번이 내보내는 `ZepetoBaseModel.fbx`에는 그 표정 모양이
**0개**라, Blender에는 섞을 것이 없고 따라서 키를 찍을 수도 없습니다. 얼굴 모양 250개는
Play 중 서버에서 내려올 때 붙습니다.

**깜빡임은 Unity 쪽에서 넣습니다.** 헬퍼 **6번(클립 조정)** 의 `얼굴 · 눈 깜빡임` 칸에
`눈 깜빡임 넣기` 버튼이 있습니다. Blender 작업을 다 끝낸 **마지막에** 누르세요 —
여기서 다시 내보내면 그 커브가 사라집니다.
